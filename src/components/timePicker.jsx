...
const genNumber = (length) => Array.from({ length }, (v, i) => genDouble(i));

const hours = genNumber(24);

const minutes = genNumber(60);
...
const TimeBox = ({ customRef = null, selected, data = [], keyName = '', handleTime }) => {
  return (
    <ul ref={customRef} className='h-60 overflow-y-scroll no-scrollbar snap-y snap-mandatory'>
      {data.map((current) => {
        return (
          <li key={current}>
            <button
...
const TimeInput = ({ time = {}, customRef, handleBlur, handleInput, handleChange, ...rest }) => {
  const { hour, minute } = time;
  return (
